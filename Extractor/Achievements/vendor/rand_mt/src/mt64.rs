...
impl Zeroize for Mt19937GenRand64 {
    fn zeroize(&mut self) {
        self.state.zeroize();
        self.idx.zeroize();
    }
...
impl Drop for Mt19937GenRand64 {
    fn drop(&mut self) {
        self.zeroize();
    }
}
...
mod tests {
    ...
    #[test]
    fn explicit_zeroize_wipes_live_state_and_index() {
        let mut mt = Mt19937GenRand64::new(0x0123_4567_89ab_cdef);
        let _ = mt.next_u64();
        assert_ne!(mt.idx, 0);
        assert!(mt.state.iter().any(|value| *value != Wrapping(0)));

        mt.zeroize();

        assert_eq!(mt.idx, 0);
        assert!(mt.state.iter().all(|value| *value == Wrapping(0)));
    }
