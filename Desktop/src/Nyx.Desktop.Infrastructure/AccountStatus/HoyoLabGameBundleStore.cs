...
public sealed class HoyoLabGameBundleStore
{
    ...
    public bool TryMigrateFromV1(
        ...
        bool rememberHsrEndgame = false,
        bool rememberZzzBuilds = false,
        bool rememberZzzEndgame = false) => SerializeMutation(
        () => TryMigrateFromV1Core(
            ...
            rememberHsrEndgame, rememberZzzBuilds, rememberZzzEndgame),
        false,
        cancellationToken);
    ...
    public bool TryRecordZzzBuilds(
        PublisherRoleBinding binding,
        HoyoLabZzzBuildSnapshot builds,
        DateTimeOffset observedAt,
        CancellationToken cancellationToken = default)
    {
        if (binding is null
            || builds is null
            || gameId != HoyoLabGameBundleRules.ZzzGameId
            || !HoyoLabZzzBuildRules.IsValid(builds)
            || !IsExactUtcSecond(observedAt))
            return false;
        return TryMutate(
            bundle => RecordZzzBuilds(bundle, binding, builds, observedAt),
            cancellationToken);
    }

    public bool TryRecordGenshinBuilds(
        PublisherRoleBinding binding,
        HoyoLabGenshinBuildSnapshot builds,
        DateTimeOffset observedAt,
        CancellationToken cancellationToken = default)
    {
        if (binding is null
            ...
            || !HoyoLabGenshinBuildRules.IsValid(builds)
            || !IsExactUtcSecond(observedAt))
            return false;
        return TryMutate(
            bundle => RecordGenshinBuilds(bundle, binding, builds, observedAt),
            cancellationToken);
    }
    ...
    public bool TryRecordGenshinEvents(
        PublisherRoleBinding binding,
        HoyoLabGenshinEventsSnapshot events,
        DateTimeOffset observedAt,
        CancellationToken cancellationToken = default)
    {
        if (binding is null
            || gameId != HoyoLabGameBundleRules.GenshinGameId
            || !HoyoLabGenshinEventsRules.IsValid(events)
            || !IsExactUtcSecond(observedAt))
            return false;
        return TryMutate(
            bundle => RecordGenshinEvents(bundle, binding, events, observedAt),
            cancellationToken);
    }

    public bool TryRecordGenshinEndgame(
        PublisherRoleBinding binding,
        HoyoLabGenshinEndgameSnapshot events,
        DateTimeOffset observedAt,
        CancellationToken cancellationToken = default)
    {
        if (binding is null
            || gameId != HoyoLabGameBundleRules.GenshinGameId
            || !HoyoLabGenshinEndgameRules.IsValid(events)
            || !IsExactUtcSecond(observedAt))
            return false;
        return TryMutate(
            bundle => RecordGenshinEndgame(bundle, binding, events, observedAt),
            cancellationToken);
    }

    public bool TryRecordZzzEndgame(
        PublisherRoleBinding binding,
        HoyoLabZzzEndgameSnapshot endgame,
        DateTimeOffset observedAt,
        CancellationToken cancellationToken = default)
    {
        if (binding is null
            || gameId != HoyoLabGameBundleRules.ZzzGameId
            || !HoyoLabZzzEndgameRules.IsValid(endgame)
            || !IsExactUtcSecond(observedAt))
            return false;
        return TryMutate(
            bundle => RecordZzzEndgame(bundle, binding, endgame, observedAt),
            cancellationToken);
    }
        ...
        CancellationToken cancellationToken = default)
    {
        if (binding is null
            || gameId != HoyoLabGameBundleRules.GameId
            || !HoyoLabHsrEventsRules.IsValid(events)
            || !IsExactUtcSecond(observedAt))
            return false;
        return TryMutate(
            bundle => RecordHsrEvents(bundle, binding, events, observedAt),
            cancellationToken);
    }
    ...
    private bool TryMigrateFromV1Core(
        ...
        bool rememberZzzBuilds,
        bool rememberZzzEndgame)
    {
        ...
        var migrated = new HoyoLabGameBundle(
            ...
            new(
                ...
                Builds: gameId == HoyoLabGameBundleRules.GenshinGameId && rememberGenshinBuilds
                    ...
                    || gameId == HoyoLabGameBundleRules.ZzzGameId && rememberZzzBuilds,
                ...
                Endgame: gameId == HoyoLabGameBundleRules.GenshinGameId && rememberGenshinEndgame
                    ...
                    || gameId == HoyoLabGameBundleRules.ZzzGameId && rememberZzzEndgame,
    ...
    private static HoyoLabGameBundle? RecordZzzBuilds(
        HoyoLabGameBundle bundle,
        PublisherRoleBinding binding,
        HoyoLabZzzBuildSnapshot builds,
        DateTimeOffset observedAt)
    {
        ...
        if (existingAt > observedAt) return null;
        if (existingAt == observedAt)
            return HoyoLabZzzBuildRules.ValuesEqual(role.ZzzBuilds, builds)
                ? bundle
                : null;
        if (!CanReplaceTombstone(
                bundle.CapabilityTombstones,
        ...
        roles[index] = role with
        {
            ...
            ZzzBuilds = HoyoLabZzzBuildRules.Normalize(builds),
        };
        return bundle with
        {
            Roles = roles,
    ...
    private static HoyoLabGameBundle? RecordGenshinBuilds(
        HoyoLabGameBundle bundle,
        PublisherRoleBinding binding,
        HoyoLabGenshinBuildSnapshot builds,
        DateTimeOffset observedAt)
    {
        ...
        if (existingAt > observedAt) return null;
        if (existingAt == observedAt)
            return HoyoLabGenshinBuildRules.ValuesEqual(role.GenshinBuilds, builds)
                ? bundle
                : null;
        if (!CanReplaceTombstone(
                bundle.CapabilityTombstones,
        ...
        {
            Observations = role.Observations with { Builds = observedAt },
            GenshinBuilds = HoyoLabGenshinBuildRules.Normalize(builds),
        };
        return bundle with
        {
            Roles = roles,
    ...
    private static HoyoLabGameBundle? RecordGenshinEvents(
        HoyoLabGameBundle bundle,
        PublisherRoleBinding binding,
        HoyoLabGenshinEventsSnapshot events,
        DateTimeOffset observedAt)
    {
        if (!bundle.Consents.Events) return null;
        var index = FindRole(bundle, binding);
        if (index < 0) return null;
        var role = bundle.Roles[index];
        var existingAt = role.Observations.Events;
        if (existingAt > observedAt) return null;
        if (existingAt == observedAt)
            return HoyoLabGenshinEventsRules.ValuesEqual(role.GenshinEvents, events)
                ? bundle
                : null;
        if (!CanReplaceTombstone(
                bundle.CapabilityTombstones,
                binding,
                HoyoLabGameBundleRules.Events,
                observedAt))
            return null;
        var roles = bundle.Roles.ToArray();
        roles[index] = role with
        {
            Observations = role.Observations with { Events = observedAt },
            GenshinEvents = HoyoLabGenshinEventsRules.Normalize(events),
        };
        return bundle with
        {
            Roles = roles,
    ...
    private static HoyoLabGameBundle? RecordGenshinEndgame(
        HoyoLabGameBundle bundle,
        PublisherRoleBinding binding,
        HoyoLabGenshinEndgameSnapshot events,
        DateTimeOffset observedAt)
    {
        ...
        if (existingAt > observedAt) return null;
        if (existingAt == observedAt)
            return HoyoLabGenshinEndgameRules.ValuesEqual(role.GenshinEndgame, events)
                ? bundle
                : null;
        if (!CanReplaceTombstone(
                bundle.CapabilityTombstones,
        ...
        roles[index] = role with
        {
            ...
            GenshinEndgame = HoyoLabGenshinEndgameRules.Normalize(events),
        };
        return bundle with
        {
            Roles = roles,
    ...
    private static HoyoLabGameBundle? RecordZzzEndgame(
        HoyoLabGameBundle bundle,
        PublisherRoleBinding binding,
        HoyoLabZzzEndgameSnapshot endgame,
        DateTimeOffset observedAt)
    {
        ...
        if (existingAt > observedAt) return null;
        if (existingAt == observedAt)
            return HoyoLabZzzEndgameRules.ValuesEqual(role.ZzzEndgame, endgame)
                ? bundle
                : null;
        if (!CanReplaceTombstone(
                bundle.CapabilityTombstones,
        ...
        {
            Observations = role.Observations with { Endgame = observedAt },
            ZzzEndgame = HoyoLabZzzEndgameRules.Normalize(endgame),
        };
        return bundle with
        {
            Roles = roles,
        ...
        DateTimeOffset observedAt)
    {
        ...
        if (existingAt > observedAt) return null;
        if (existingAt == observedAt)
            return HoyoLabHsrEventsRules.ValuesEqual(role.HsrEvents, events)
                ? bundle
                : null;
        if (!CanReplaceTombstone(
                bundle.CapabilityTombstones,
        ...
        {
            Observations = role.Observations with { Events = observedAt },
            HsrEvents = HoyoLabHsrEventsRules.Normalize(events),
        };
        return bundle with
        {
            Roles = roles,
    ...
    private IReadOnlyList<string> SupportedCapabilities() =>
        gameId == HoyoLabGameBundleRules.ZzzGameId
            ? [HoyoLabGameBundleRules.Resources, HoyoLabGameBundleRules.Builds, HoyoLabGameBundleRules.Endgame]
            : gameId == HoyoLabGameBundleRules.GameId
    ...
    private static HoyoLabCapabilityConsentSet SetConsent(
        ...
        bool value) => capability switch
        {
            ...
            HoyoLabGameBundleRules.Events => consents with { Events = value },
            HoyoLabGameBundleRules.Endgame => consents with { Endgame = value },
    ...
    private static HoyoLabGameBundleRole ClearCapability(
        ...
        {
            HoyoLabGameBundleRules.Resources => role with
            {
                Observations = role.Observations with { Resources = null },
                Resource = null,
            },
            HoyoLabGameBundleRules.Builds => role with
            {
            ...
            HoyoLabGameBundleRules.Endgame => role with
            {
            ...
            {
                Observations = role.Observations with { Events = null },
                GenshinEvents = null,
                HsrEvents = null,
            },
    ...
    private static DateTimeOffset? ObservationFor(
        ...
        string capability) => capability switch
        {
            ...
            HoyoLabGameBundleRules.Events => role.Observations.Events,
            HoyoLabGameBundleRules.Endgame => role.Observations.Endgame,
        ...
        Action<ReadOnlyMemory<byte>>? clearedBufferObserver)
    {
        ...
        using (var writer = new Utf8JsonWriter(owner, new JsonWriterOptions { Indented = false }))
        {
            ...
            foreach (var role in bundle.Roles)
            {
                ...
                if (role.GenshinBuilds is { } builds)
                {
                ...
                if (role.GenshinEvents is { } events)
                {
                    writer.WritePropertyName("genshinEvents");
                    events.Data.WriteTo(writer);
                }
                if (role.GenshinEndgame is { } endgame)
                {
                    writer.WritePropertyName("genshinEndgame");
                    endgame.Data.WriteTo(writer);
                }
                if (role.ZzzBuilds is { } zzzBuilds)
                {
                    writer.WritePropertyName("zzzBuilds");
                    zzzBuilds.Data.WriteTo(writer);
                }
                if (role.ZzzEndgame is { } zzzEndgame)
                {
                    writer.WritePropertyName("zzzEndgame");
                    zzzEndgame.Data.WriteTo(writer);
                }
        ...
        out HoyoLabGameBundle? bundle)
    {
        ...
        try
        {
            using var document = JsonDocument.Parse(plaintext, new JsonDocumentOptions
            {
                ...
                // Bundle > role > Abyss period > floor > chamber > battle > avatar.
                MaxDepth = 16,
            });
        ...
        out IReadOnlyList<HoyoLabGameBundleRole> roles)
    {
        ...
        foreach (var item in element.EnumerateArray())
        {
            ...
            var hasZzzBuilds = item.TryGetProperty("zzzBuilds", out var zzzBuildsElement);
            var hasZzzEndgame = item.TryGetProperty("zzzEndgame", out var zzzEndgameElement);
            ...
            if (hasZzzBuilds && gameId == HoyoLabGameBundleRules.ZzzGameId) fields.Add("zzzBuilds");
            if (hasZzzEndgame && gameId == HoyoLabGameBundleRules.ZzzGameId) fields.Add("zzzEndgame");
            ...
            if (!HasExactProperties(item, fields.ToArray())
                ...
                || !TryParseAchievementIds(
                 ...
                 || hasZzzBuilds && !TryParseZzzBuilds(zzzBuildsElement, gameId, out zzzBuilds)
                 || hasZzzEndgame && !TryParseZzzEndgame(zzzEndgameElement, gameId, out zzzEndgame))
                return false;
            parsed.Add(new(
    ...
    private static bool TryParseZzzBuilds(
        JsonElement element,
        string gameId,
        out HoyoLabZzzBuildSnapshot? builds)
    {
        builds = null;
        if (gameId != HoyoLabGameBundleRules.ZzzGameId
            || element.ValueKind != JsonValueKind.Object)
            return false;
        var candidate = new HoyoLabZzzBuildSnapshot(element.Clone());
        if (!HoyoLabZzzBuildRules.IsValid(candidate)) return false;
        builds = candidate;
        return true;
    }

    private static bool TryParseGenshinBuilds(
        JsonElement element,
        string gameId,
        out HoyoLabGenshinBuildSnapshot? builds)
    {
        builds = null;
    ...
    private static bool TryParseGenshinEvents(
        JsonElement element,
        string gameId,
        out HoyoLabGenshinEventsSnapshot? events)
    {
        events = null;
        if (gameId != HoyoLabGameBundleRules.GenshinGameId) return false;
        var candidate = new HoyoLabGenshinEventsSnapshot(element);
        if (!HoyoLabGenshinEventsRules.IsValid(candidate)) return false;
        events = HoyoLabGenshinEventsRules.Normalize(candidate);
        return true;
    }

    private static bool TryParseGenshinEndgame(
        JsonElement element,
        string gameId,
        out HoyoLabGenshinEndgameSnapshot? events)
    {
        events = null;
        if (gameId != HoyoLabGameBundleRules.GenshinGameId) return false;
    ...
    private static bool TryParseZzzEndgame(
        JsonElement element,
        string gameId,
        out HoyoLabZzzEndgameSnapshot? endgame)
    {
        endgame = null;
        if (gameId != HoyoLabGameBundleRules.ZzzGameId) return false;
        var candidate = new HoyoLabZzzEndgameSnapshot(element);
        if (!HoyoLabZzzEndgameRules.IsValid(candidate)) return false;
        endgame = HoyoLabZzzEndgameRules.Normalize(candidate);
        return true;
    }
    ...
    private static bool TryParseHsrEvents(
        JsonElement element,
        string gameId,
        out HoyoLabHsrEventsSnapshot? events)
    {
        events = null;
        if (gameId != HoyoLabGameBundleRules.GameId) return false;
        var candidate = new HoyoLabHsrEventsSnapshot(element);
        if (!HoyoLabHsrEventsRules.IsValid(candidate)) return false;
        events = HoyoLabHsrEventsRules.Normalize(candidate);
        return true;
    }
