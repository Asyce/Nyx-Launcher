...
public static class HoyoLabGameBundleRules
{
        ...
        foreach (var tombstone in bundle.CapabilityTombstones)
        {
            ...
            if (tombstone.Capability == Events
                && (activeRole.GenshinEvents is not null
                    || activeRole.HsrEvents is not null
    ...
    public static HoyoLabGameBundle Normalize(HoyoLabGameBundle bundle) => bundle with
    {
        Roles = bundle.Roles.Select(static role => role with
        {
            ...
            HsrEvents = role.HsrEvents is null
                ? null
                : HoyoLabHsrEventsRules.Normalize(role.HsrEvents),
        }).ToArray(),
        ...
        DateTimeOffset utcNow)
    {
        if (role is null
            ...
            || !IsValidTimestamp(role.Observations.Endgame, utcNow)
        ...
        if (role.Observations.Exploration is not null)
        {
            if (gameId != GenshinGameId
                || !consents.Exploration
                || !HoyoLabGenshinExplorationRules.IsValid(role.GenshinExploration))
                return false;
        }
        else if (role.GenshinExploration is not null)
        {
            return false;
        }
        ...
        if (role.Observations.Events is not null)
        {
            if (!consents.Events
                ...
                || gameId == GameId
                    && (role.GenshinEvents is not null || !HoyoLabHsrEventsRules.IsValid(role.HsrEvents)))
                return false;
        }
        else if (role.GenshinEvents is not null || role.HsrEvents is not null)
        {
            return false;
        }
