...
public sealed partial class PublisherAccountService
{
    ...
    {
        if (!GenshinExplorationAvailable || !HoyoLabAccountSlotRules.IsValidSlotId(expectedSlotId)
            ...
            return new(HoyoLabGenshinExplorationReadStatus.NotEnabled);
        ...
        try
        {
            ...
            if (before?.Consents.Exploration != true || before.SelectedRole != expectedBinding
            ...
            if (proof != PublisherSessionProof.Authenticated)
                return new(proof == PublisherSessionProof.LoginRequired
                    ? HoyoLabGenshinExplorationReadStatus.LoginRequired
                    : HoyoLabGenshinExplorationReadStatus.NeedsReview);

            var result = await window.ReadGenshinExplorationAsync(expectedBinding, token);
            if (result.Status != HoyoLabGenshinExplorationReadStatus.Completed || result.Snapshot is null)
                return result;
            ...
            lock (sync)
            {
                ...
                if (current?.Consents.Exploration != true || current.SelectedRole != expectedBinding
                    || TryLoadRoleRecord("gi", operation)?.Binding != expectedBinding)
                    return new(HoyoLabGenshinExplorationReadStatus.Canceled);
                var observedAt = DateTimeOffset.FromUnixTimeSeconds(DateTimeOffset.UtcNow.ToUnixTimeSeconds());
                if (!genshinGameBundle.TryRecordGenshinExploration(expectedBinding, result.Snapshot, observedAt, token))
                    return new(HoyoLabGenshinExplorationReadStatus.LocalStorageUnavailable);
        ...
        {
            // A failed refresh retains the last complete exploration observation.
