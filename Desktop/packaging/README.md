...
No user information, alternate port, query, fragment, other host, other scheme, or path variation is accepted. The updater itself has no downloader. The installed stable app checks the fixed Pengo manifest endpoint after its first frame, asks before downloading an upgrade, and hands the verified local files to the updater. The transport rejects redirects and checks the sealed package size and SHA-256; the updater rechecks the whole-package SHA-256 before extraction and every file SHA-256 while extracting. Development packages do not enter this installed-stable update flow.
...
Development and stable artifacts are both unsigned output. A stable channel label does not make a file trusted or published. Windows SmartScreen may warn. Authenticode signing is a separate owner-provisioned release step requiring a publisher identity, protected code-signing certificate/account, timestamping and verification; it is not configured by these scripts.

Unsigned publication is supported: pushing a version tag triggers the stable package build and, after the hosted Windows checks pass, creates the GitHub release. Do not push a release tag before the native and exact-package install/update/rollback acceptance gates are complete. Publishing an unsigned package does not establish Windows publisher trust.
