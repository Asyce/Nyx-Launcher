...
public sealed class HoyoLabSyncStateStoreTests
{
    ...
    [Fact]
    public void Schema_four_preserves_existing_opt_ins_and_pending_conditions_without_enabling_zzz_or_rewriting_on_read()
    {
        ...
        using var deletion = new HoyoLabPendingDeletion(credential.SyncId, credential.Token,
            HoyoLabSyncStateStore.AllHoyoScope, "legacy-condition", Now, requireRevisionMatch: true,
            expectedRevisionsByGame: new(Now.AddMinutes(-1), null));
        using var source = new HoyoLabSyncState(credential, Now, [deletion],
        ...
        legacy["automaticSync"]!.AsObject().Remove("zzzEnabled");
        legacy["automaticSync"]!.AsObject().Remove("zzzLastSyncedAt");
        legacy["pendingDeletions"]![0]!["expectedRevisionsByGame"]!.AsObject().Remove("zzz");
        ...
        Assert.Equal(source.AutomaticSync, loaded.AutomaticSync);
        Assert.False(loaded.AutomaticSync.ZzzEnabled);
        Assert.Null(loaded.AutomaticSync.ZzzLastSyncedAt);
        Assert.Null(Assert.Single(loaded.PendingDeletions).ExpectedRevisionsByGame!.Zzz);
        Assert.Equal(before, File.ReadAllBytes(store.StatePath));
        Assert.True(store.TrySetWorkerRevision(Now.AddMilliseconds(1)));
        using var promoted = Assert.IsType<HoyoLabSyncState>(store.TryLoad());
        Assert.Equal(source.AutomaticSync, promoted.AutomaticSync);
        Assert.Equal(5, StateJson(promoted)["schemaVersion"]!.GetValue<int>());
    }

    [Fact]
    public void Zzz_preferences_success_times_and_game_deletion_are_isolated()
    {
        using var root = new TemporaryRoot();
        var store = CreateStore(root.Path);
        using var credential = Credential(1);
        ...
        {
            Assert.True(store.TrySetAutomaticSync(game, true));
            Assert.True(store.TryRecordSuccessfulSync(game, credential, Now));
        }
        using var before = Assert.IsType<HoyoLabSyncState>(store.TryLoad());
        Assert.True(before.AutomaticSync.ZzzEnabled);
        Assert.Equal(Now, before.AutomaticSync.ZzzLastSyncedAt);
        using var deletion = new HoyoLabPendingDeletion(credential.SyncId, credential.Token, "zzz", "delete-zzz", Now);
        Assert.True(store.TryQueueGameDeletion(deletion));
        using var after = Assert.IsType<HoyoLabSyncState>(store.TryLoad());
        Assert.False(after.AutomaticSync.ZzzEnabled);
        Assert.Null(after.AutomaticSync.ZzzLastSyncedAt);
        Assert.True(after.AutomaticSync.HsrEnabled);
        Assert.True(after.AutomaticSync.GenshinEnabled);
        Assert.Equal(Now, after.AutomaticSync.HsrLastSyncedAt);
        Assert.Equal(Now, after.AutomaticSync.GenshinLastSyncedAt);
        Assert.Equal(credential.SyncId, after.CurrentCredential!.SyncId);
    }

    [Fact]
    public void Zzz_role_deletion_round_trips_both_capture_cutoffs_and_rejects_unsupported_events()
    {
        using var credential = Credential(1);
        var binding = new PublisherRoleBinding("123456789", "prod_gf_eu");
        using var deletion = new HoyoLabPendingRoleDeletion(credential.SyncId, credential.Token, credential.Key,
            binding, "zzz-role", Now, null, null, Now, "zzz",
            knownBuildsAt: Now.AddMinutes(-2), knownEndgameAt: Now.AddMinutes(-1));
        ...
        using var loaded = parsed!;
        var role = Assert.Single(loaded.PendingRoleDeletions);
        Assert.Equal("zzz", role.GameId);
        Assert.Equal(deletion.KnownBuildsAt, role.KnownBuildsAt);
        Assert.Equal(deletion.KnownEndgameAt, role.KnownEndgameAt);
        Assert.Throws<ArgumentException>(() => new HoyoLabPendingRoleDeletion(credential.SyncId, credential.Token,
            credential.Key, binding, "zzz-events", Now, null, null, Now, "zzz", knownEventsAt: Now.AddMinutes(-1)));
    }
    ...
    public void Schema_one_loads_without_role_intents_and_next_write_promotes_exact_schema_five(string scope)
    {
        using var root = new TemporaryRoot();
        var store = CreateStore(root.Path);
        using var credential = Credential(1);
    ...
    [Fact]
    public void Schema_two_loads_and_rewrites_to_schema_five_without_dropping_intents()
    {
        using var root = new TemporaryRoot();
        var store = CreateStore(root.Path);
        using var credential = Credential(1);
    ...
    public void Automatic_sync_defaults_off_and_enable_requires_credential_without_pending_work()
    {
        ...
        using (var loaded = Assert.IsType<HoyoLabSyncState>(store.TryLoad()))
            Assert.Equal(new HoyoLabAutomaticSyncSettings(), loaded.AutomaticSync);
        Assert.False(store.TrySetAutomaticSync("unknown", true));
        Assert.False(store.TrySetAutomaticSync("unknown", false));
    ...
    [Fact]
    public void Current_schema_preserves_strict_per_game_revision_map()
    {
        using var root = new TemporaryRoot();
        var store = CreateStore(root.Path);
        using var credential = Credential(1);
        ...
        Assert.Equal(["gi", "hsr", "zzz"], map.Select(property => property.Key).Order(StringComparer.Ordinal));
    ...
    [Fact]
    public void Genshin_role_build_cutoff_round_trips_omits_null_legacy_field_and_is_part_of_equality()
    {
        using var root = new TemporaryRoot();
        var store = CreateStore(root.Path);
        using var credential = Credential(1);
        ...
        var scalarRole = json.DeepClone().AsObject();
        scalarRole["pendingRoleDeletions"]![0] = 1;
        var scalarBytes = Encoding.UTF8.GetBytes(scalarRole.ToJsonString());
        try
        {
            Assert.False(HoyoLabSyncStateStore.TryParseState(scalarBytes, Now, out _));
        }
        finally { CryptographicOperations.ZeroMemory(scalarBytes); }

        var nullCutoff = json.DeepClone().AsObject();
        nullCutoff["pendingRoleDeletions"]![0]!["knownBuildsAt"] = null;
        var nullBytes = Encoding.UTF8.GetBytes(nullCutoff.ToJsonString());
        try
        {
            Assert.False(HoyoLabSyncStateStore.TryParseState(nullBytes, Now, out _));
        }
        ...
        try
        {
            Assert.False(HoyoLabSyncStateStore.TryParseState(schemaTwoBytes, Now, out _));
        }
        ...
        try
        {
            ...
            Assert.Equal(Now.AddMinutes(-1), Assert.Single(parsedState.PendingRoleDeletions).KnownBuildsAt);
        }
        finally { CryptographicOperations.ZeroMemory(bytes); }

        var legacy = json.DeepClone().AsObject();
        legacy["pendingRoleDeletions"]![0]!.AsObject().Remove("knownBuildsAt");
        var legacyBytes = Encoding.UTF8.GetBytes(legacy.ToJsonString());
        try
        {
            Assert.True(HoyoLabSyncStateStore.TryParseState(legacyBytes, Now, out var parsed));
            using var parsedState = Assert.IsType<HoyoLabSyncState>(parsed);
            var legacyRole = Assert.Single(parsedState.PendingRoleDeletions);
            Assert.Null(legacyRole.KnownBuildsAt);
            var rewritten = HoyoLabSyncStateStore.SerializeState(parsedState);
            try { Assert.Equal(legacyBytes, rewritten); }
    ...
    [Fact]
    public void Genshin_role_events_cutoff_round_trips_only_when_present_and_is_part_of_equality()
    {
        using var root = new TemporaryRoot();
        var protector = new TrackingProtector();
        var store = CreateStore(root.Path, protector);
        ...
        using var clone = deletion.Clone();
        Assert.Equal(deletion.KnownEventsAt, clone.KnownEventsAt);
        Assert.Equal(deletion.Binding, clone.Binding);
        Assert.True(store.TryEnqueuePendingRoleDeletion(deletion));
        using var loaded = Assert.IsType<HoyoLabSyncState>(store.TryLoad());
        var saved = Assert.Single(loaded.PendingRoleDeletions);
        Assert.Equal(Now.AddMinutes(-1), saved.KnownEventsAt);
        var json = StateJson(loaded);
        var item = json["pendingRoleDeletions"]![0]!.AsObject();
        Assert.Equal(FormatTimestamp(Now.AddMinutes(-1)), item["knownEventsAt"]!.GetValue<string>());
        ...
        try
        {
            Assert.True(HoyoLabSyncStateStore.TryParseState(bytes, Now, out var parsed));
            using var parsedState = Assert.IsType<HoyoLabSyncState>(parsed);
            Assert.Equal(Now.AddMinutes(-1), Assert.Single(parsedState.PendingRoleDeletions).KnownEventsAt);
        }
        finally { CryptographicOperations.ZeroMemory(bytes); }

        var encrypted = File.ReadAllBytes(store.StatePath);
        Assert.Equal(-1, encrypted.AsSpan().IndexOf(Encoding.UTF8.GetBytes("knownEventsAt")));
        Assert.Contains(protector.ProtectedPlaintextSnapshots, plaintext =>
            Encoding.UTF8.GetString(plaintext).Contains("knownEventsAt", StringComparison.Ordinal));

        var absent = json.DeepClone().AsObject();
        absent["pendingRoleDeletions"]![0]!.AsObject().Remove("knownEventsAt");
        var absentBytes = Encoding.UTF8.GetBytes(absent.ToJsonString());
        try
        {
            Assert.True(HoyoLabSyncStateStore.TryParseState(absentBytes, Now, out var parsed));
            using var parsedState = Assert.IsType<HoyoLabSyncState>(parsed);
            var parsedDeletion = Assert.Single(parsedState.PendingRoleDeletions);
            Assert.Null(parsedDeletion.KnownEventsAt);
            var rewritten = HoyoLabSyncStateStore.SerializeState(parsedState);
            try { Assert.Equal(absentBytes, rewritten); }
            finally { CryptographicOperations.ZeroMemory(rewritten); }
        }
        ...
        var nullField = json.DeepClone().AsObject();
        nullField["pendingRoleDeletions"]![0]!["knownEventsAt"] = null;
        var nullBytes = Encoding.UTF8.GetBytes(nullField.ToJsonString());
        try { Assert.False(HoyoLabSyncStateStore.TryParseState(nullBytes, Now, out _)); }
        finally { CryptographicOperations.ZeroMemory(nullBytes); }
    ...
    public void Role_endgame_cutoff_round_trips_only_when_present_and_is_part_of_equality(string gameId, string server)
    {
        ...
        using var clone = deletion.Clone();
        Assert.Equal(deletion.KnownEndgameAt, clone.KnownEndgameAt);
        Assert.Equal(deletion.Binding, clone.Binding);
        Assert.True(store.TryEnqueuePendingRoleDeletion(deletion));
        using var loaded = Assert.IsType<HoyoLabSyncState>(store.TryLoad());
        var saved = Assert.Single(loaded.PendingRoleDeletions);
        Assert.Equal(Now.AddMinutes(-1), saved.KnownEndgameAt);
        var json = StateJson(loaded);
        var item = json["pendingRoleDeletions"]![0]!.AsObject();
        Assert.Equal(FormatTimestamp(Now.AddMinutes(-1)), item["knownEndgameAt"]!.GetValue<string>());
        ...
        try
        {
            Assert.True(HoyoLabSyncStateStore.TryParseState(bytes, Now, out var parsed));
            using var parsedState = Assert.IsType<HoyoLabSyncState>(parsed);
            Assert.Equal(Now.AddMinutes(-1), Assert.Single(parsedState.PendingRoleDeletions).KnownEndgameAt);
        }
        finally { CryptographicOperations.ZeroMemory(bytes); }

        var encrypted = File.ReadAllBytes(store.StatePath);
        Assert.Equal(-1, encrypted.AsSpan().IndexOf(Encoding.UTF8.GetBytes("knownEndgameAt")));
        Assert.Contains(protector.ProtectedPlaintextSnapshots, plaintext =>
            Encoding.UTF8.GetString(plaintext).Contains("knownEndgameAt", StringComparison.Ordinal));

        var absent = json.DeepClone().AsObject();
        absent["pendingRoleDeletions"]![0]!.AsObject().Remove("knownEndgameAt");
        var absentBytes = Encoding.UTF8.GetBytes(absent.ToJsonString());
        try
        {
            Assert.True(HoyoLabSyncStateStore.TryParseState(absentBytes, Now, out var parsed));
            using var parsedState = Assert.IsType<HoyoLabSyncState>(parsed);
            var parsedDeletion = Assert.Single(parsedState.PendingRoleDeletions);
            Assert.Null(parsedDeletion.KnownEndgameAt);
            var rewritten = HoyoLabSyncStateStore.SerializeState(parsedState);
            try { Assert.Equal(absentBytes, rewritten); }
            finally { CryptographicOperations.ZeroMemory(rewritten); }
        }
        ...
        var nullField = json.DeepClone().AsObject();
        nullField["pendingRoleDeletions"]![0]!["knownEndgameAt"] = null;
        var nullBytes = Encoding.UTF8.GetBytes(nullField.ToJsonString());
        try { Assert.False(HoyoLabSyncStateStore.TryParseState(nullBytes, Now, out _)); }
        finally { CryptographicOperations.ZeroMemory(nullBytes); }
        ...
        using var changed = new HoyoLabPendingRoleDeletion(
            ...
            deletion.DeletedAt,
            deletion.GameId,
            knownEndgameAt: Now.AddMinutes(-2));
        Assert.False(store.TryEnqueuePendingRoleDeletion(changed));
    ...
    private static void AssertDefaultAutomaticSync(JsonObject json)
    {
        ...
        Assert.Equal(6, automatic.Count);
        ...
        Assert.False(automatic["zzzEnabled"]!.GetValue<bool>());
        Assert.Null(automatic["zzzLastSyncedAt"]);
    }
