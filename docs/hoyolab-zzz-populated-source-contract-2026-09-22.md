...
Create a wholly synthetic shared fixture exercising all null/empty and awakening/skin/stat-precision variants. Define a complete, explicit equipped-build projection and separately scoped Shiyu projection. Add ZZZ to protected game bundles, game-specific encrypted AAD, merge/deletion/consent handling, sync state and receiver routing only after tracing every two-game assumption and testing old HSR/GI compatibility. New native availability must remain gated until receiver publication and integrated own-role capture/sync acceptance. Browser source observation does not prove installed launcher capture or automatic upload.
...
The new code is not connected to WebView sessions, protected game bundles, My HoYo, manual sync or automatic sync. Supported sync games remain HSR and Genshin. Before enabling ZZZ, extend the game-specific bundle validation, AES-GCM associated data, merge and tombstones, protected state migration, conditional all-game deletion, recovery/rotation and UI routing. Specifically test that a legacy two-game delete condition treats an omitted ZZZ revision as expected absent and conflicts if ZZZ data exists. Preserve old clients and unrelated game bundles. Publish and verify receiver support before native availability, then obtain installed native capture/sync acceptance.
